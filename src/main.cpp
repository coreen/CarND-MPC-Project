...
int main() {
  ...
  h.onMessage([&mpc](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    std::string sdata = string(data).substr(0, length);
    cout << sdata << endl;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < ptsx.size(); i++) {
            // shift reference angle by 90 degrees
          ...
          */
          ...
          double y0 = 0;
          double psi0 = 0;
          double delay = 100 / 1000; // 100 ms converted to sec
          ...
          msgJson["steering_angle"] = actuators[0] / (deg2rad(25) * Lf);
          ...
          for (int i = 1; i < num_points; i++) {
            double inc = poly_inc * i;

            next_x_vals.push_back(inc);
            next_y_vals.push_back(polyeval(coeffs, inc));
          }
